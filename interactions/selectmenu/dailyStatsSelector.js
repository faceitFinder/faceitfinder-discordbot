  ...
  async execute(interaction) {
    ...
    if (values.userId === interaction.user.id)
      try {
        ...
        const to = (new Date(values.date * 1000).setHours(24)).toString().slice(0, -3)
        const playerHistory = await Player.getHistory(playerId, values.maxMatch, values.date, parseInt(to))
        const playerStats = await generatePlayerStats(playerHistory, playerId)

        const graphCanvas = await Graph.generateCanvas(playerId, values.maxMatch, null, parseInt(to))
        const elo = await Graph.getElo(playerId, values.maxMatch, null, parseInt(to))
        console.log(elo)
        ...
        console.log(playerStats.wins, playerStats.games)
        ...
        const card = new Discord.MessageEmbed()
          ...
          .addFields({ name: 'From', value: new Date(values.date * 1000).toDateString(), inline: false },
            { name: 'Games', value: `${elo.length} (${(playerStats.wins / playerStats.games) * 100}% Win)`, inline: true },
