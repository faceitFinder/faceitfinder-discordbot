...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const searchForCurrentUser = currentPlayer.faceitId !== playerAimed
    ...
    if (!faceitIncluded.length && !steamIncluded.length) {
      faceitIncluded = (await getUsers(interaction, 5, null, 'faceit_parameters', false, searchForCurrentUser)).map(e => e.param)
      steamIncluded = (await getUsers(interaction, 5 - faceitIncluded.length, 'steam_parameters', null, searchForCurrentUser)).map(e => e.param)
    }
